...
def computeDelta(wt, X, Xi):
    ...
    for i in xrange(0,len(Xi.index)):
        Xi_row = Xi.iloc[i]
        Yi = Xi_row["Yi"]
        Xi_row = Xi_row.drop("Yi")
        stdXi = Xi_row.std(axis = 0)
        Xi_row_mean = Xi_row.mean(axis = 0)
        numerator = 0
        for j in xrange(0,len(Xi_row.index)):
            numerator = numerator + ((X[j]-X_mean)*(Xi_row[j]-Xi_row_mean))
        denomenator = len(Xi_row.index)*stdX*stdXi
        S = math.exp(wt*(numerator/denomenator))
        E_numerator = E_numerator + Yi*S
        E_denomenator = E_denomenator + S
    E = E_numerator/E_denomenator
    return E
